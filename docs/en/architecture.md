...
| Worker | Claude Code or Codex | Implement one approved Beads task | Expand scope or close task before review |
...
humanize/RLCR is the default Claude worker path. `claude.humanize=required` makes non-YOLO `humanize-flow run` fail closed when no humanize integration is detected and instructs Claude to start RLCR before editing code. Teams can lower this to `auto` or `off` when a repository or environment cannot support humanize for a specific run.

Codex worker mode is deliberately narrower. `worker.provider=codex` only applies to `run --yolo`, uses `worker.codex.model` and `worker.codex.reasoning_effort`, and never starts humanize/RLCR. This keeps the quota-saving path explicit while preserving the separate Codex review gate.
