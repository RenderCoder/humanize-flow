...
面向人类的生成产物默认使用英文，可通过 `humanize-flow i18n <lang>` 切换。这包括 `bd-plan.md` 等 Markdown 产物、handoff prose、Beads 标题/描述/验收标准、实现总结、review、PR 文本和 commit message 正文。路径、命令、API 名称、JSON 字段名、枚举值、label、Beads ID 和代码标识符等会被工具消费的技术字面量保持原始形式。
