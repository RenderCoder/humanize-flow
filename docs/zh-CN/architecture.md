...
| Worker | Claude Code 或 Codex | 实现一个已批准 Beads 任务 | 扩大范围或 review 前关闭任务 |
...
humanize/RLCR 是默认 Claude worker 路径。`claude.humanize=required` 会让非 YOLO 的 `humanize-flow run` 在检测不到 humanize 集成时 fail closed，并要求 Claude 在改代码前启动 RLCR。若某个仓库或环境无法支持 humanize，团队可以把单次运行或全局默认值降为 `auto` 或 `off`。

Codex worker 模式刻意保持更窄。`worker.provider=codex` 只作用于 `run --yolo`，使用 `worker.codex.model` 和 `worker.codex.reasoning_effort`，并且不会启动 humanize/RLCR。这样可以显式节省 Claude 额度，同时保留独立的 Codex review 门禁。
