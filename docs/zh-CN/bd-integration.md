...
如果希望实现改动不落在主 checkout 中，使用 `humanize-flow run-next --worktree`。CLI 选出 ready 任务后会检查 `bd worktree info`；如果当前目录还不是 Beads worktree，它会用 `bd worktree create` 创建 `../feature-<bd-id>`，分支为 `feature/<bd-id>`，并在新 worktree 中继续执行 worker 命令。
