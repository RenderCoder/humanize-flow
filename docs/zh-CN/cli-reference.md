...
从主仓库 checkout 开始实现、并希望每个任务隔离在 Beads 管理的 worktree 中时，可以使用 `--worktree`：
...
如果当前目录已经是 `bd worktree`，`--worktree` 不会再创建新目录，worker 会原地运行。否则 CLI 会通过 `bd worktree create` 创建 `../feature-<bd-id>`，分支名为 `feature/<bd-id>`，然后在新 worktree 中继续执行同一条 `run` 命令。这样 Beads 状态会通过 Beads redirect 文件共享，同时避免在主 checkout 中直接写实现改动。
...
传入 `--worktree` 时，`run-next` 会先选择 ready 任务，再采用和 `run --worktree` 相同的 Beads worktree 行为。
